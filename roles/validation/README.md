...
| `validation_check_oauth_apps_connectivity` | HTTPS GET `https://oauth-openshift.apps.<cluster>.<domain>/` (API NLB :443 express lane); non-fatal warning on failure | `true` |
...
6. **`oauth-openshift.apps` HTTPS** — reachability via API NLB listener **443** / `*-oauth-tg`; see [issues #23](https://github.com/iron-gryphon/gryphon-forge/issues/23) and [#24](https://github.com/iron-gryphon/gryphon-forge/issues/24). Non-fatal warning and report line when the probe fails.
