...
| `csr_approver_deregister_bootstrap_from_api_mcs_after_complete` | After `wait-for bootstrap-complete`, deregister the bootstrap instance from `<cluster>-api-tg` and `<cluster>-mcs-tg` (AWS API delegated to the controller). OAuth uses a separate `<cluster>-oauth-tg` on the API NLB; see **aws_nodes** and [issues #23](https://github.com/iron-gryphon/gryphon-forge/issues/23)/[#24](https://github.com/iron-gryphon/gryphon-forge/issues/24). | true |
...
3. **TCP connectivity** — Ports **6443** and **22623** on `api-int` must be reachable (MCS on 22623 serves master Ignition; same NLB as API in gryphon-forge). The API NLB also exposes **443** for the OAuth hostname express lane (`oauth-openshift.apps` in Route53).
