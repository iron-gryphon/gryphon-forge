...
| `csr_approver_repair_oauth_openshift_route` | **Bounded OpenShift-only workaround** ([issue #23](https://github.com/iron-gryphon/gryphon-forge/issues/23)): during `install-complete`, patch `oauth-openshift` Route if `targetPort` is wrongly `6443` (must match the **Service** port for `443`, usually name `https`). This is **not** an AWS NLB or target-group fix — ingress LB correctness is owned by **aws_nodes**. Set to `false` to disable the patch loop while debugging transport or LB-only issues. Uses Service discovery when `csr_approver_repair_oauth_openshift_route_target_port` is empty. Exponential backoff (or fixed interval), throttled warnings, and caps apply as below. Revert cycles append to `{{ install_dir }}/.gryphon-oauth-route-revert.log`. If caps are hit, the script fails with capture instructions (see [issue #24](https://github.com/iron-gryphon/gryphon-forge/issues/24) when the Route is already correct but TLS/router errors persist). | true |
| `csr_approver_oauth_route_repair_diagnostics` | When `csr_approver_repair_oauth_openshift_route` runs a patch, log Route `targetPort` and TLS mode, `oauth-openshift` Service ports, `openshift-ingress` router pods, and (if `aws` is on `PATH`, `foundry_region` is set, and credentials work) NLB ingress target-group health for `*-ingress-443-tg` / `*-ingress-80-tg`. | true |
