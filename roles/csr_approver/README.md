...
| `csr_approver_emit_console_ingress_hints_on_install_failure` | When `wait-for install-complete` fails, print short console/ingress troubleshooting hints; see **AGENTS.md** | false |
...
| `csr_approver_deregister_bootstrap_from_api_mcs_after_complete` | After `wait-for bootstrap-complete`, deregister the bootstrap instance from `<cluster>-api-tg` and `<cluster>-mcs-tg` (AWS API delegated to the controller). When **`forge_oauth_apps_via_api_nlb`** is true, OAuth also uses `<cluster>-oauth-tg` on the API NLB; see **aws_nodes** and [issues #23](https://github.com/iron-gryphon/gryphon-forge/issues/23)/[#24](https://github.com/iron-gryphon/gryphon-forge/issues/24)/[#30](https://github.com/iron-gryphon/gryphon-forge/issues/30). | true |
...
3. **TCP connectivity** — Ports **6443** and **22623** on `api-int` must be reachable (MCS on 22623 serves master Ignition; same NLB as API in gryphon-forge). The API NLB may expose **443** for the OAuth express lane when **`forge_oauth_apps_via_api_nlb`** is true (older OCP default).
