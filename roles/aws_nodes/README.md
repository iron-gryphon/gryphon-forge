...
| `aws_nodes_lb_prune_api_mcs_worker_targets` | Deregister instances tagged `role=worker` or `gpu-worker` for this cluster (they must not receive API/MCS traffic) | `true` |
| `aws_nodes_lb_api_mcs_deregister_bootstrap` | Deregister running bootstrap from API/MCS (set `true` after control plane serves API/MCS and bootstrap is no longer required) | `false` |
| `aws_nodes_lb_api_mcs_prune_extra_instance_ids` | Operator-confirmed instance IDs to deregister (never applied to `role=master` for this cluster) | `[]` |
| `aws_nodes_lb_api_mcs_prune_stale_non_master_targets` | Deregister missing/terminated targets and non-running non-bootstrap/non-master instances | `true` |
| `aws_nodes_lb_prune_api_mcs_foreign_cluster_targets` | Deregister when `tag:cluster` is set and does not match `cluster_name` | `true` |
...
| `aws_nodes_dns_gate_enabled` | Before worker/GPU EC2, assert `api` / `api-int` exist in the private zone and the zone is associated with `foundry_vpc_id` | `true` |
| `aws_nodes_dns_gate_vpc_resolver_ip` | Optional Amazon VPC DNS IP for extra `dig` checks (e.g. `10.1.0.2`); overrides foundry | `""` |
| `aws_nodes_dns_gate_derive_vpc_resolver_from_vault_cidr` | When explicit IP and `foundry_vault_amazon_provided_dns` are empty, derive resolver from `foundry_vault_vpc_cidr` (same formula as foundry `vault_vpc_amazon_provided_dns`) | `true` |
| `aws_nodes_dns_gate_vpc_resolver_query` | Run `community.general.dig` against that resolver from the controller (needs UDP/53 reachability, e.g. VPN) | `false` |
| `aws_nodes_dns_gate_delegate_vpc_resolver_query_to_bastion` | Run `dig` from the bastion host against the Vault VPC resolver | `false` |
...
4. **EC2 control plane** (`ec2_control_plane.yml`): Resolve `infraID` (see table above), tag Forge bootstrap/master/worker SGs with **`kubernetes.io/cluster/<infraID>=owned`** for AWS CCM when ENIs use multiple groups, then launch **bootstrap and master** nodes only (same tag on instances plus `Name` / `cluster` / `role`). Bootstrap and master ENIs use `<cluster>-bootstrap-sg` / `<cluster>-master-sg` **plus** optional `foundry_vault_api_security_group_id` and `foundry_vault_security_group_id` when foundry outputs supply them. Each launch supplies a **single** root `volumes` entry at `aws_nodes_rhcos_root_device_name_resolved` (AMI `RootDeviceName`) with the configured gp3 size.
5. **Load Balancers**: Create one internal **API NLB** for **api-int** with listeners **6443** (Kubernetes API) and **22623** (Machine Config Server). When **`aws_nodes_oauth_apps_alias_api_nlb`** is true (default for OCP versions before 4.21), the API NLB also listens on **443** and forwards to **`{{ cluster_name }}-oauth-tg`** (masters :6443). Masters and workers pull Ignition from `https://api-int:22623/config/...`, so **22623 must be on the same hostname** Route53 aliases to (`api` / `api-int` → `{{ cluster_name }}-api`). A separate `{{ cluster_name }}-mcs` NLB is removed on apply (legacy). Ingress uses ALB (443/80) or NLB. NLBs use **cross-zone load balancing** so traffic from any AZ can reach bootstrap in one subnet. **Bootstrap and masters** register on **both** `{{ cluster_name }}-api-tg` (6443) and `{{ cluster_name }}-mcs-tg` (22623), using the same instance ID set (EC2 launch results or `ec2_instance_info` when using `--tags load_balancers`). A follow-on **prune** pass (`load_balancers_api_mcs_prune.yml`) deregisters workers, foreign-cluster instances, terminated bootstrap, and operator-supplied IDs from those target groups only — **masters for this cluster are never removed**. After the control plane serves MCS, set `aws_nodes_lb_api_mcs_deregister_bootstrap: true` (or terminate bootstrap; terminated bootstrap is removed automatically when stale pruning is enabled) to clear live bootstrap targets. If you run only `--tags load_balancers`, registration uses `ec2_instance_info` with retries until nodes appear. **Ingress NLB/ALB:** Forge registers **all** worker (and GPU worker) instances on ingress target groups while the default `IngressController` often runs **two** router pods; expect **some targets unhealthy** in `{{ cluster_name }}-ingress-443-tg` / `-ingress-80-tg` until pods spread or you scale replicas — this is normal if `*.apps` DNS aliases only to the Forge ingress load balancer and HTTP(S) probes succeed on nodes that host `router-default`.
6. **Route53**: Register api, api-int, and *.apps records (aliases to the API NLB and ingress LB) in `foundry_internal_hosted_zone_id`. Record names use **`cluster_name`** and **`base_domain`** after `deploy_cluster.yml` normalization (including foundry `ocp_cluster_name` / `ocp_base_domain` overrides) so they match install-config and Ignition URLs. **`oauth-openshift.apps.<cluster>.<domain>`** uses **`forge_oauth_apps_via_api_nlb`** / **`aws_nodes_oauth_apps_alias_api_nlb`**: **API NLB** for versions before 4.21 (issues #23/#24), **same target as `*.apps`** (ingress) for 4.21 and later ([issue #30](https://github.com/iron-gryphon/gryphon-forge/issues/30)). When the API path is enabled, **kube-apiserver** must present a cert for that hostname — see **`ignition_oauth_apps_api_named_certificate`** and **AGENTS.md**.
7. **DNS gate before workers** (`dns_gate_before_workers.yml`, tag `dns_gate`, skipped when `worker_count` and `gpu_worker_count` are both zero): Uses `amazon.aws.route53_info` to confirm the private hosted zone is **associated with `foundry_vpc_id`** (so instances in the Vault VPC resolve names via AmazonProvidedDNS at *VPC CIDR network + 2*, e.g. `10.1.0.2` for `10.1.0.0/16`) and that **`api` / `api-int` A/alias records** exist before any worker or GPU worker EC2 launch. When gryphon-foundry emits **`vault_vpc_amazon_provided_dns`**, optional resolver checks use that value before falling back to CIDR derivation. When **`ocp_api_int_fqdn`** is set, Forge asserts it equals `api-int.<cluster_name>.<base_domain>`. Optional: `dig` against the VPC resolver from the controller (`aws_nodes_dns_gate_vpc_resolver_query`) or from the bastion (`aws_nodes_dns_gate_delegate_vpc_resolver_query_to_bastion`). The same **VPC ↔ zone** association is asserted in `playbooks/preflight.yml` when validating foundry JSON (requires AWS credentials).
8. **EC2 workers** (`ec2_workers.yml`): Launch worker and optional GPU worker instances after DNS is in place.
9. **Optional root volume verification** (`ec2_root_volume_verify.yml`): When `aws_nodes_verify_root_ebs_size_after_launch` is true, assert root EBS size for bootstrap, masters, and any workers launched in this run.
10. **LB validation snapshot** (when `aws_nodes_lb_validation_enabled`): Writes `forge-lb-validation/...` artifacts under `install_dir`. Optional bastion DNS/TCP probes (`bastion-diagnostics.txt`) run only when `aws_nodes_lb_validation_from_bastion` is true (default false). On failure, the **csr_approver** role runs the same probes via `bastion_lb_diagnostics.yml` into the failure log directory or `csr-fail/<cluster>-<epoch>/` under the bastion install path.

### Private zones, VPC DNS, and ingress (ALB / private-only DNS)

**Requirement:** `foundry_internal_hosted_zone_id` must refer to a **private** hosted zone **associated with the Vault VPC** (`foundry_vpc_id`). Otherwise RHCOS will query the VPC resolver and get **NXDOMAIN** for `api-int.<cluster>.<base_domain>` even if records exist in another zone.

**Optional OpenShift configuration:** When ingress uses an internal ALB (`foundry_ingress_certificate_arn`) or you manage DNS outside the cluster (private-only records), align with the OpenShift Networking guide for your version (for example [Configuring ingress cluster traffic on AWS](https://docs.redhat.com/en/documentation/openshift_container_platform/4.15/html/networking/configuring-ingress-cluster-traffic#configuring-ingress-cluster-traffic-aws)): set **`dnsManagementPolicy: Unmanaged`** on the `IngressController` and choose an **`endpointPublishingStrategy`** (for example `LoadBalancerService` with an internal NLB/ELB) so the platform does not expect public DNS updates you are not making. Apply with your preferred GitOps flow or a one-off manifest; Forge does not patch `IngressController` by default.
